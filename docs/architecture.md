...
5. **Dialog** — Uživatel vidí náhled, vybere plugin (dropdown obsahuje všechny providery) a **submission mode** (Timed / Aggregated; radio button módu, který `ProviderInfo.SupportedModes` neobsahuje, je disabled přes `CanUseTimedMode` / `CanUseAggregatedMode`) a potvrdí. Volba se persistuje per plugin: `ApplicationSettings.SubmissionModeByProvider` (pluginId → mode) plus `LastSubmissionProviderId` pro plugin, na kterém se dialog otevře; `LastSubmissionMode` zůstává globálním fallbackem pro plugin, který v mapě ještě není. Výběr pluginu přehodí mode na ten jeho (`ResolveModeFor`), nepodporovaný zapamatovaný mode se ignoruje. V **Aggregated** módu orchestrátor před zobrazením preview seskupí DTO podle `(TicketId, Description)` per den a součtem nastaví `DurationMinutes`; `StartTime` skupiny = nejstarší start.
